...
class RwmemTestBase(unittest.TestCase):
    ...
    def assertOutput(self, rwmemopts, expected):
        ...
        res = subprocess.run(
            [self.rwmem_cmd, *opts], capture_output=True, encoding='ASCII', check=False
        )
...
class RwmemNumericReadTests(RwmemTestBase):
    ...
    def test_numeric_reads_single(self):
        self.assertOutput(['0'], '0x00 = 0x7d8c0c39\n')

        self.assertOutput(['0x10'], '0x10 (+0x0) = 0x8ee570d6\n')

        self.assertOutput(['-d', '8', '0'], '0x00 = 0x39\n')

        self.assertOutput(['-d', '16', '0'], '0x00 = 0x0c39\n')

        self.assertOutput(['-d', '16', '0'], '0x00 = 0x0c39\n')

        self.assertOutput(['-d', '32', '0'], '0x00 = 0x7d8c0c39\n')

        self.assertOutput(['-d', '24', '0'], '0x00 = 0x8c0c39\n')

        self.assertOutput(['-d', '24be', '0'], '0x00 = 0x390c8c\n')
...
class RwmemNumericWriteTests(RwmemTestBase):
    ...
    def test_numeric_writes_single(self):
        self.assertOutput(['0x0=0'], '0x00 = 0x7d8c0c39 := 0x00000000 -> 0x00000000\n')

        self.assertOutput(['0xa0=0'], '0xa0 (+0x0) = 0x24a91022 := 0x00000000 -> 0x00000000\n')

        self.assertOutput(
            ['-d', '24', '0xb0=0x123456'], '0xb0 (+0x0) = 0xd8b5dc := 0x123456 -> 0x123456\n'
        )
...
class RwmemRegisterDatabaseTests(RwmemTestBase):
    ...
    def test_regdb_list(self):
        ...
        res = subprocess.run(
            [self.rwmem_cmd, 'list', '--regs=' + TEST_REGDB_PATH],
            capture_output=True,
            encoding='ASCII',
            check=False,
        )
    ...
    def test_regdb_list_search_sensor_a(self):
        ...
        res = subprocess.run(
            [self.rwmem_cmd, 'list', '--regs=' + TEST_REGDB_PATH, 'SENSOR_A'],
            capture_output=True,
            encoding='ASCII',
            check=False,
        )
    ...
    def test_regdb_list_search_sens_wildcard(self):
        ...
        res = subprocess.run(
            [self.rwmem_cmd, 'list', '--regs=' + TEST_REGDB_PATH, 'SENS*'],
            capture_output=True,
            encoding='ASCII',
            check=False,
        )
    ...
    def test_regdb_list_search_sensor_a_stat_wildcard(self):
        ...
        res = subprocess.run(
            [self.rwmem_cmd, 'list', '--regs=' + TEST_REGDB_PATH, 'SENSOR_A.STAT*'],
            capture_output=True,
            encoding='ASCII',
            check=False,
        )
    ...
    def test_regdb_field_access(self):
        ...
        self.assertOutput(
            ['SENSOR_A.CONFIG_REG:THRESHOLD'],
            'SENSOR_A.CONFIG_REG            0x04 = 0x00344772\n'
            + '  THRESHOLD                      23:16 = 0x00000034 \n',
        )

        self.assertOutput(
            ['SENSOR_A.STATUS_REG:MODE'],
            'SENSOR_A.STATUS_REG            0x00 = 0x00000039\n'
            + '  MODE                            7:3  = 0x00000007 \n',
        )
