...
class UnpackedField:
    def __init__(self, name: str, high: int, low: int) -> None:
        self._validate_inputs(name, high, low)
        self.name = name
        self.high = high
        self.low = low

    def _validate_inputs(self, name: str, high: int, low: int) -> None:
...
class UnpackedRegister:
    def __init__(self, name: str, offset: int, fields: Sequence[UnpackedField | tuple] | None = None) -> None:
        self._validate_inputs(name, offset)
        self.name = name
        self.offset = offset
    ...
    def _validate_inputs(self, name: str, offset: int) -> None:
    ...
    def validate_against_block(self, block_name: str, block_offset: int, block_size: int, data_size: int) -> None:
        """Validate register against its containing block."""
        # Check if register fits within block boundaries (if block size is specified)
        if block_size > 0:
            reg_end = self.offset + data_size
            if reg_end > block_size:
                raise RegisterValidationError(
                    f"Register '{self.name}' in block '{block_name}': "
                    f"register extends beyond block boundary (offset {self.offset} + size {data_size} > block size {block_size})"
                )

        # Validate fields against register data width
        for field in self.fields:
            field.validate_against_register_width(self.name, data_size)


class UnpackedRegBlock:
    def __init__(self, name: str, offset: int, size: int, regs: Sequence[UnpackedRegister | tuple],
                 addr_endianness: Endianness, addr_size: int, data_endianness: Endianness, data_size: int) -> None:
        self._validate_inputs(name, offset, size, addr_endianness, addr_size, data_endianness, data_size)
    ...
    def _validate_inputs(self, name: str, offset: int, size: int, addr_endianness: Endianness,
                        addr_size: int, data_endianness: Endianness, data_size: int) -> None:
    ...
    def _validate_register_overlaps(self) -> None:
        ...
        reg_ranges = [(r.offset, r.offset + self.data_size, r.name) for r in self.regs]
    ...
    def _validate_registers_against_block(self) -> None:
        ...
        for reg in self.regs:
            reg.validate_against_block(self.name, self.offset, self.size, self.data_size)


class UnpackedRegFile:
    def __init__(self, name: str, blocks: Sequence[UnpackedRegBlock | tuple]) -> None:
    ...
    def _validate_inputs(self, name: str) -> None:
